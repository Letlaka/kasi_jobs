...
Next steps (optional):
- Re-run audit against a generated lockfile or `requirements.txt` for exact declared-deps coverage.
- Add `pip-audit` and `safety` to CI (GitHub Actions) for continuous monitoring.

Generated files are in the `reports/` folder.

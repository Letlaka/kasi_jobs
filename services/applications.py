...
from applications.models import ACTION_ACCEPT, ACTION_REJECT, Application, ApplicationAction
...
def accept_application(application: Application, user: object) -> None:
    ...
    try:
        with transaction.atomic():
            locked_app = (
                Application.objects.select_for_update()  # type: ignore[attr-defined]
                .select_related("job")
                .get(pk=app_pk)
            )
            ...
            if status is not None and status != Application.ApplicationStatus.PENDING:
                raise ApiError(
            ...
            prev_status = status
            locked_app.status = Application.ApplicationStatus.ACCEPTED
            locked_app.save()

            # persist audit record
            ApplicationAction.objects.create(  # type: ignore[attr-defined]
...
def reject_application(application: Application, user: object) -> None:
    logger = get_logger(__name__)
    start = time.monotonic()
    ...
    try:
        with transaction.atomic():
            ...
            prev_status = status
            locked_app.status = Application.ApplicationStatus.REJECTED
            locked_app.save()
